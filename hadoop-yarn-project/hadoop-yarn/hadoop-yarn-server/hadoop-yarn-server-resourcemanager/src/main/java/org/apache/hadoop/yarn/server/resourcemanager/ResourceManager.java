...
public class ResourceManager extends CompositeService
  ...
  // todo 在main函数resourceManager.init(conf)最终调用的是serviceInit方法
  @Override
  ...
  protected Dispatcher createDispatcher() {
    // todo AsyncDispatcher其实就是一个事件类型的生产者消费者模型
  ...
  /**
   * 这里最核心的其实就是：资源初始化与启动服务
   */
  public static void main(String argv[]) {
    // todo 设定主线程出现未定义捕获处理的异常时的handler
    ...
    try {
      ...
      if (argv.length >= 1) {
        ...
        // todo 把RM的CompositeService的shutdownHook添加到一个统一的ShutdownHookManager
        ShutdownHookManager.get().addShutdownHook(
          new CompositeServiceShutdownHook(resourceManager),
          SHUTDOWN_HOOK_PRIORITY);
        // todo 初始化
        // todo 这里就是调用AbstractService.init，然后调用ResourceManager.serviceInit
  ...
  private Dispatcher setupDispatcher() {
    // todo 设置Dispatcher
    // todo 实际上就是通过createDispatcher()方法创建了一个AsyncDispatcher实例，代码如下
